...
(function ($) {
    ...
    function setSize(container, param) {
        ...
        function setVSize(pp, type) {
            ...
            pp.panel("move", {left: 0, top: (type == "n" ? 0 : cc.height() - height)});
            cpos.height -= height;
            if (type == "n") {
                cpos.top += height;
        ...
        function setHSize(pp, type) {
            ...
            pp.panel("move", {left: (type == "e" ? cc.width() - width : 0), top: cpos.top});
            cpos.width -= width;
            if (type == "w") {
                cpos.left += width;
                if (!opts.split && opts.border) {
    ...
    function addPanel(container, param, el) {
        ...
        panel.resizable($.extend({}, {
            handles: (handles[dir] || ""), disabled: (!pp.panel("options").split), onStartResize: function (e) {
                resizing = true;
    ...
    function collapsePanel(container, region, animateSpeed) {
        ...
        function createExpandPanel(dir) {
            var iconMap = {"east": "left", "west": "right", "north": "down", "south": "up"};
            ...
            p.panel($.extend({}, $.fn.layout.paneldefaults, {
                ...
                onResize: function () {
                    ...
                    if (ptitle.length) {
                        ...
                        ptitle.css({left: (left + "px"), top: (top + "px")});
                    }
                }
            }));
        ...
        function getOption() {
            ...
            if (region == "east") {
                ...
                return {
                    resizeC: {width: cwidth},
                    expand: {left: cc.width() - pwidth},
                    expandP: {top: copts.top, left: cc.width() - csize, width: csize, height: copts.height},
                    collapse: {left: cc.width(), top: copts.top, height: copts.height}
                };
            } else {
                if (region == "west") {
                    var pwidth = p.panel("panel")._outerWidth();
                    ...
                    return {
                        resizeC: {width: cwidth, left: csize - 1},
                        expand: {left: 0},
                        expandP: {left: 0, top: copts.top, width: csize, height: copts.height},
                        collapse: {left: -pwidth, top: copts.top, height: copts.height}
                    };
                } else {
                    if (region == "north") {
                        ...
                        return {
                            resizeC: {top: csize - 1, height: hh},
                            expand: {top: 0},
                            expandP: {top: 0, left: 0, width: cc.width(), height: csize},
                            collapse: {top: -pheight, width: cc.width()}
                        };
                    } else {
                        if (region == "south") {
                            var pheight = p.panel("panel")._outerHeight();
                            ...
                            return {
                                resizeC: {height: hh},
                                expand: {top: cc.height() - pheight},
                                expandP: {top: cc.height() - csize, left: 0, width: cc.width(), height: csize},
        ...
        function getOption() {
            ...
            if (region == "east" && panels.expandEast) {
                ...
                if (region == "west" && panels.expandWest) {
                    return {
                        collapse: {left: -p.panel("panel")._outerWidth(), top: copts.top, height: copts.height},
                        expand: {left: 0}
                    };
                } else {
    ...
    $.fn.layout.defaults = {
        fit: false, onExpand: function (region) {
