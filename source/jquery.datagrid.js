...
(function ($) {
    ...
    function fixRowHeight(target, index, forceFix) {
        ...
        function setHeight(trs1, trs2) {
            for (var i = 0; i < trs2.length; i++) {
                ...
                if (height != tmpHeight) {
                    height = Math.max(height, tmpHeight) + 1;
    ...
    function wrapGrid(target, rownumbers) {
        ...
        panel.panel({doSize: false, cls: "datagrid"});
    ...
    function buildGrid(target) {
        ...
        panel.panel($.extend({}, opts, {
            id: null, doSize: false, onResize: function (width, height) {
        ...
        cells.each(function () {
            $(this).resizable({
                ...
                onStartResize: function (e) {
                    ...
                    state.proxy.css({left: e.pageX - $(panel).offset().left - 1 + e.data.deltaEdge, display: "none"});
                    setTimeout(function () {
                        if (state.proxy) {
                            state.proxy.show();
                        }
                ...
                onResize: function (e) {
                    state.proxy.css({left: e.pageX - $(panel).offset().left - 1 + e.data.deltaEdge, display: "block"});
                    return false;
                },
    ...
    function fitColumns(target) {
        ...
        var inner = header.children("div.datagrid-header-inner");
    ...
    function request(target, params, cb) {
        ...
        if (opts.pagination) {
            $.extend(param, {page: opts.pageNumber || 1, rows: opts.pageSize});
        }
            ...
            return {
                init: function (container, options) {
                    ...
                    if (isA(target, name)) {
                        ...
                        $(target)._size({width: width, height: $.fn.datagrid.defaults.editorHeight});
                    }
                }
            };
        };
    ...
    $.fn.datagrid.methods = {
        options: function (jq) {
            ...
            return jq.each(function () {
                ...
                $(target).datagrid("getPager").pagination("refresh", {pageNumber: page});
    ...
    var defaultView = {
        render: function (target, container, frozen) {
            ...
            body2.find("tbody *").css({height: 1, borderColor: "transparent", background: "transparent"});
    ...
    $.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
        ...
        rownumberWidth: 30,
        editorHeight: 31,
        headerEvents: {mouseover: headerOverEventHandler(true), mouseout: headerOverEventHandler(false), click: headerClickEventHandler, dblclick: headerDblclickEventHandler, contextmenu: headerMenuEventHandler},
        rowEvents: {mouseover: hoverEventHandler(true), mouseout: hoverEventHandler(false), click: clickEventHandler, dblclick: dblclickEventHandler, contextmenu: contextmenuEventHandler},
